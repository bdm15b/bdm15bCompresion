 ...
 * 
 *  Note: This files tests all folders in src/files/lzwTests
 * 
 * ------------- DESIGN -------------
 *  Overview/Process
 *      1. Delete all .ll files in test dir
 *      2. Store test file contents in a map
 *      3. Compress test files → creating .*.ll compressed files
 ...
 *          - multiple files
 *  
 *  NOTE: if you do not want to keep compressed files, then uncomment the last 
...
public class SchubsLTest {
    ...
    public void singleFileTest() throws IOException {
        ...
        // removes all ".ll" files from huffmanTests directory
        cleanTestDir("ll");

        Map<String, String> originalContents = new HashMap<String, String>();
        String testDirPath =  "src" + File.separator + "files" + File.separator + "lzwTests";
        ...
        for (String relativeFilepath : files) {
            ...
            // compress file
            SchubsL.main(new String[] { filepath });

            // delete original file
            deleteFile(filepath);

            // decompress file
            Deschubs.main(new String[] { filepath + ".ll" });
    ...
    public void multipleFileTest() throws IOException {
        ...
        for (int i = 1; i < files.length; i++) {
            ...
            for (String fileTobeDecompressed : test)
                Deschubs.main(new String[] { fileTobeDecompressed + ".ll" });
